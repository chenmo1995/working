...
public class TestDP {
    ...
    /**
     * 746. 使用最小花费爬楼梯
     * 到达第 k 层，需要花费的力气是 min(dp[k-1],dp[k-2])+nums[k]
     * <p>
    ...
    /**
     * 96. 不同的二叉搜索树
     * dp[i] 表示 i 个节点组成的不同二叉搜索树的个数
     * 每个数都可以作为根节点，根据不同的数作为根节点，可以写出递推公式
    ...
    /**
     * 416. 分割等和子集
     * 可以理解成挑选一定数量的元素，装满容量为 sum/2 的背包
     * 背包容量为 sum/2 的背包能装的最大价值是不是 sum/2
     *
     * @param nums
     * @return
     */
    ...
    public int lastStoneWeightII2(int[] stones) {
        // dp[j] 表示容量为 j 的背包最多，最多可以背的石头的重量

        int sum = 0;
        for (int stone : stones) {
            sum += stone;
        }
        // target 就是背包的最大容量；因为是分为两堆，一堆是 dp[target]，另一堆就是 sum - dp[target]
        // 背包最大目标重量也就是 sum 的一半
        int target = sum / 2;
        ...
        // target 向下取整的，sum - dp[target] > dp[target]
        return sum - dp[target] - dp[target];
    }

    /**
     * 518. 零钱兑换 II
     * 容量为 amount 的背包，能装的最大价值的硬币是多少，硬币无限个，妥妥的完全背包
     * <p>
     * 但是求得是组合数，递推公式有所不同
     ...
     * 322. 零钱兑换
     * 求最少的硬币个数
     * 硬币可以多次使用，装满 amount 的最小硬币个数
     * dp[i][j] 表示容量为 j，装满需要的最小硬币个数
     * 单独考虑第 i 个硬币用不用
     * 不用：dp[i][j] = dp[i - 1][j]
     * 用：dp[i][j] = dp[i][j - weight[i]] + 1
     * 求最小个数，那肯定就是去 min(a, b) 了
     * 先遍历物品还是背包，都可以。但是背包的遍历要顺序
     *
     * @param coins
     * @param amount
     * @return
    ...
    /**
     * 279. 完全平方数
     * 完全平方数是物品，n 为背包，物品完全装满背包，的最小个数
     * dp[i][j] 表示前 i 个物品装满容量为 j 的背包需要的最小物品个数
     * 装第 i 个物品：dp[i][j] = dp[i][j - i] + 1
    ...
    /**
     * 139. 单词拆分
     * 其实都很难想到使用动态规划；现在就告诉你应该用背包问题来思考，你怎么做
     * s 作为背包，wordDict 作为物品，判断能否用物品装满背包 s，且物品可以重复使用
     * 感觉是完全背包。
     * <p>
     * 先用记忆化搜索来做，主体是回溯
     *
    ...
    public boolean wordBreak2(String s, List<String> wordDict) {
        ...
        return backTracing(s, set, memory, 0);
    }

    private boolean backTracing(String s, Set<String> set, int[] memory, int startIndex) {
        if (startIndex == s.length()) {
            return true;
        }
        // 从 s[startIndex] 开始，后面的元素是否能由 set 构成；记录在 memory 里面，-1,0,1 分别表示，不可以，未处理，可以
        ...
        for (int i = startIndex; i < s.length(); i++) {
            ...
            if (set.contains(temp) && backTracing(s, set, memory, i + 1)) {
                // 这里其实不用记录可以组成 s 的情况，因为这时候直接返回就好了，这里本来就是为了快速返回结果的
                // 只需要记录 s[startIndex] 不满足条件的情况就好了
                memory[startIndex] = 1;
                return true;
            }
        }
        // s[startIndex] 后面的元素不能由 set 构成
        memory[startIndex] = -1;
        return false;
    }
     ...
     * 139. 单词拆分
     * <p>
     * dp[i] 表示 s[0, i]  是否可以由 wordDict 组成
     * 如果 dp[j] 为 1，且 s[j, i] 在 wordDict 中，那么 dp[i] = 1
     *
     * @param s
     * @param wordDict
     * @return
    ...
    /**
     * 198. 打家劫舍
     * dp[i] 表示有 i 户人，偷到的最高金额
     * - 偷第 i 户人：dp[i] = dp[i - 2] + nums[i]
     * - 不偷第 i 户人：dp[i] = dp[i - 1];
     *
     * @param nums
     * @return
     */
    ...
    /**
     * 213. 打家劫舍 II
     * 因为第一个数和最后一个数不能同时取到，所以可以分成【不考虑第一个数】和【不考虑最后一个数】这两种情况，然后取最大值
     * <p>
    ...
    public int rob22(int[] nums) {
        ...
        int a = robMax(nums, 0, n - 2);
        int b = robMax(nums, 1, n - 1);
        return Math.max(a, b);
    }
     ...
     * <p>
     * 比如 i = 4,就可以分为 1、2、3、4 分别做根节点
     * 1 做根节点，右边 3 个节点，左边 0 个   dp[3] * dp[0]
     * 2 做根节点，右边 2 个节点，左边 1 个   dp[2] * dp[1]
     * 3 做根节点，右边 1 个节点，左边 2 个   dp[1] * dp[2]
     * 4 做根节点，右边 0 个节点，左边 3 个   dp[0] * dp[3]
     * 全部加起来即可
     *
     * @param n
     * @return
    ...
    public int change(int amount, int[] coins) {
        ...
        for (int i = 0; i <= amount; i++) {
            for (int j = 0; j < coins.length; j++) {//5,2,1
                /* 后遍历物品的时候，容量为 3 ，物品 {1,2} 和 {2,1}，其实计算了两次。
                   访问 1 的时候，dp[3] = dp[3] + dp[3 - 1]
                   访问 2 的时候，dp[3] = dp[3] + dp[3 - 2]
                   下面的 dp[3] 加上了 {2,1}，上面的 dp[3] 加上了 {1,2}
                */
                /* 但是先遍历物品的话，
                    先访问 1 的时候，dp[3] 绝无可能访问到 2
                    所以只会在访问 2 的时候，访问到 1。{1,2} 和 {2,1} 只会出现一次
                 */
                if (i - coins[j] >= 0) {
    ...
    public String longestPalindrome(String s) {
        int n = s.length();
        // dp[i][j] 表示位置 i 到位置 j 的子字符串是不是回文字符串
        int[][] dp = new int[n][n];
        dp[0][0] = 1;
        for (int i = 1; i < n; i++) {
            dp[i][i - 1] = 1;
